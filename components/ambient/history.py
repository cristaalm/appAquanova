...
class TableCard(QFrame):
    def __init__(self, historial, rango_max, rango_min, parent=None,table_height=300):  
    ...
    def populate_table(self): 
        ...
        for row, item in enumerate(page_data):
            ...
            temp_item = QTableWidgetItem(f'{item["valor"]:.1f}')
            temp_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.history_table.setItem(row, 1, temp_item)

            # Columna 3: Estado temperatura
            estado_temp_item = QTableWidgetItem(item.get("estado_temp", "N/A"))
            estado_temp_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.history_table.setItem(row, 2, estado_temp_item)

            # Columna 4: Humedad
            humedad_item = QTableWidgetItem(f'{item["valor"]:.1f}')
            humedad_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.history_table.setItem(row, 3, humedad_item)
                
            # Columna 5: Estado ambiente
            estado_amb_item = QTableWidgetItem(item.get("estado_ambiente", "N/A"))
            estado_amb_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.history_table.setItem(row, 4, estado_amb_item)
